...
bin/logstash --pipeline.workers 7 --path.data ./products_data -f "$PRODUCTS_LOGSTASH_FILE" 2>&1 | tee products_indexing.log
...
bin/logstash --pipeline.workers 7 --path.data ./query_data -f "$QUERIES_LOGSTASH_FILE" 2>&1 | tee queries_indexing.log